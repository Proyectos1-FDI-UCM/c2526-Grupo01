...
/// </summary>
public class Gancho1 : MonoBehaviour
{
    [SerializeField]
    ...
    /// <summary>
    /// Inicializa las posiciones del gancho, el Rigidbody2D, y los estados iniciales
    /// </summary>
    private void Start()
    {
    ...
    /// <summary>
    /// Detecta la colisión con el jugador. Si al jugador se le ocurriera entrar por encima del gancho, este no se engancharía
    /// </summary>
    ...
    /// <summary>
    /// Método para el movimiento del gancho
    /// </summary>
    void FixedUpdate()
    {
        if (tieneJugador)
        {
            ...
            if (jugadorTransform != null)
            {
                ...
                if (rbJugador != null)
                {
                    // Ajusta la posición del jugador para que este este en el centro y justo debajo del gancho, da la casualidad que con 0.2 y -1 queda perfecto (se puede ajustar)
                    Vector3 posicionDestinoJugador = (Vector3)nuevaPos + new Vector3(0.2f, -1f, 0);
                    // Mueve al jugador con el gancho al mismo tiempo, para evitar el clippeo
                    rbJugador.MovePosition(posicionDestinoJugador);
                    // Detiene la acumulación de velocidad (para que al desenganchar al jugador este no caiga inmediatamente) y evita el clippeo
                    rbJugador.linearVelocity = Vector3.zero;
                }
            }
            // Pongo 1,4 para que el gancho tarde un poco más en soltar al jugador, para que así el jugador tenga más tiempo para pensar en lo que hacer y sepa 
            // a donde debe ir, si no es lo deseado se puede cambiar perfectamente
            if (progreso >= 1.4f)
            {
                // Cuando el jugador se desengancha, vaciamos el jugadorTransform para que el gancho este libre, y el jugador pueda moverse por su cuenta
                if (jugadorTransform != null)
                {
